...
public class BookingController {
	...
	public ResponseEntity<?> userSpecificBooking(@PathVariable("userID") String userID,
		...
		try {
			...
			// Catch any exception (mysql, numconversion) threw by the method
			// and output them into a bad request
		} catch (IndexOutOfBoundsException indexE) {
			return ResponseEntity.badRequest().body("Internal Error (Index Error)");
	...
	/*
	 * @RequestMapping(value="/booking/checkAvailability",
	 * method=RequestMethod.GET) public ResponseEntity<?>
	 * retrieveSeatsAvailableOnPeriodOfTime(
	 * 
	 * @RequestParam(value="location") String location,
	 * 
	 * @RequestParam(value="startDate") String startDate,
	 * 
	 * @RequestParam(value="endDate") String endDate){ try { Object obj =
	 * BookingDAO.getAvailableSeatsLocationDateRange(location, startDate,
	 * endDate); if(obj!=null){ return ResponseEntity.ok((List<Integer>)obj); }
	 * else{ return ResponseEntity.badRequest().body("Empty Object"); } //Catch
	 * any exception (mysql, numconversion) threw by the method and output them
	 * into a bad request }catch (Exception e) { return
	 * ResponseEntity.badRequest().body(e.toString()); } }
	 */

	// @CrossOrigin(origins = "*")
	// @RequestMapping(value="/booking/checkSingleAvailability",
	...
	// try {
	// List<BookingTable>
	// obj=BookingDAO.getIndividualSeatsAvailabilityForLocationDateRange(location,
	// startDate, endDate);
	// return (obj!=null && obj.size()>0)? ResponseEntity.ok(obj) :
	// ResponseEntity.badRequest().body("Invalid Location");
	//
	// //Catch any exception (mysql, numconversion) threw by the method and
	// output them into a bad request
	// }catch(ParseException ex){
	// return ResponseEntity.badRequest().body("Error while finding the numbers
	// of days between the given dates");
...
//	public ResponseEntity<?> retrieveBookingLength(@RequestParam(value = "startDate") String startDate,
//			@RequestParam(value = "endDate") String endDate) {
//		try {
...
//	}
	...
	public ResponseEntity<?> retrieveAllSeatsLocation(@RequestParam(value = "location") String location) {
		try {
	...
	public ResponseEntity<?> retrieveSeatData(@RequestParam(value = "deskId") String deskId) {
		try {
	...
	public @ResponseBody ResponseEntity<String> createBooking(@RequestParam int userID, @RequestParam int deskID,
			@RequestParam String startDate, @RequestParam String endDate) throws SQLException, ParseException {
	...
	public @ResponseBody ResponseEntity<String> updateBooking(@PathVariable int bookingID,
			@RequestParam String newStartDate, @RequestParam String newEndDate) throws SQLException, ParseException {
	...
	public @ResponseBody ResponseEntity<String> test(@RequestBody BookingTableWrapper bookingTableWrapper) {
		...
		String str = "";
		for(BookingTable bt : bookingTableWrapper.getBookingTables()){
			str += "ID: " + bt.getDeskID() + " | " + bt.getDates().toString();
		}
